...
func (sr *sensorRepository) GetByNodeUUID(nid string) (s []model.Sensor, err error) {
	return s, sr.db.Table("node_sensors").Select("*").Joins("join sensors on sensors.uuid=sensor_uuid").Joins("join sensor_values on sensor_values.sensor_uuid=sensors.uuid").Where("node_uuid=?", nid).Scan(&s).Error
}
